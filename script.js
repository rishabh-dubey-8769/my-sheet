...
function createQuestionNode(qn) {
  ...
  // Platform button vs Invisible alignment placeholder
  const isPlatformActive = qn.platform && qn.platform !== '*' && qn.platformUrl && qn.platformUrl !== '#';
  const logoSrc = qn.platform === 'LeetCode' ? 'Leetcode_Logo.png' : 'GFG_Logo.png';

  const platformButtonHtml = isPlatformActive ? `
  ...
  ` : `
    <div class="platform-btn platform-btn-placeholder" aria-hidden="true"></div>
  `;
